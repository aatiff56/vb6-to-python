...
def GetRegValuesToConnection(strMCn, ArrParams, FromWeb=False):
    ...
    try:
        ...
        if res == '':
            res = registry_values.one['Provider']
        strRes = 'Provider=' + res.strip() + ';'

        res = registry_values.one['Persist Security Info']
        strRes = strRes + 'Persist Security Info=' + res.strip() + ';'

        res = QueryValue('emerald', 'RTCommandActivation')
        ...
        if CommandActivation:
            ...
            if res == '':
                ...
                strRes = strRes + 'Password=' + 'Hobbit14' + ';'
            else:
                # strRes = strRes + 'Password=' + MdlConnection.PasswordDecipher(res) + ';'
                strRes = strRes + 'Password=' + 'Hobbit14' + ';'
            if len(ArrParams) >= 0:
...
def GetRegValuesToMetaConnection(Arr, FromWeb=False):
    ...
    try:
        res = QueryValue('emerald', 'Provider')
        if res == '':
            MdlConnection.SetRegMetaValues()
        res = QueryValue('emerald', 'Provider')
        strRes = 'Provider=' + res.strip() + ';'
        res = QueryValue('emerald', 'Persist Security Info')
        strRes = strRes + 'Persist Security Info=' + res.strip() + ';'
        res = QueryValue('emerald', 'RTCommandActivation')
        if res == '' or res == '0' or FromWeb == True:
            CommandActivation = False
        ...
        if CommandActivation:
            if len(Arr) >= 0:
                ...
                res = ''
            if res == '':
                res = QueryValue('emerald', 'Data Source')
            strRes = strRes + 'Data Source=' + res.strip() + ';'
            if len(Arr) >= 0:
                res = Arr[3]
            else:
                res = ''
            if res == '':
                res = QueryValue('emerald', 'User ID')
            strRes = strRes + 'User ID=' + res.strip() + ';'
            if len(Arr) >= 0:
                res = Arr[4]
            else:
                res = ''
            if res == '':
                res = QueryValue('emerald', 'Password')
                # strRes = strRes + 'Password=' + MdlConnection.PasswordDecipher(res) + ';'
                strRes = strRes + 'Password=' + 'Hobbit14' + ';'
            else:
                # strRes = strRes + 'Password=' + MdlConnection.PasswordDecipher(res) + ';'
            ...
            if len(Arr) >= 0:
                ...
                res = ''
            if res == '':
                res = QueryValue('emerald', 'MetaCatalog')
                strRes = strRes + 'Initial Catalog=' + res.strip() + ';'
            else:
                strRes = strRes + 'Initial Catalog=' + res.strip() + ';'
        else:
            ...
            res = QueryValue('emerald', 'MetaCatalog')
            strRes = strRes + 'Initial Catalog=' + res.strip() + ';'
        res = QueryValue('emerald', 'Data Provider')
        strRes = strRes + 'Data Provider=' + res.strip()
        returnVal = strRes
...
def GetConnectionStrings(strCn, strMetaCn, strSChCon, ArrParams, strERPCon=[''], FromWeb=False):
    res = ""
    returnVal = False    

    try:
        res = QueryValue('one', 'SQLDateOrder')
        if res == '':
            SQLDateOrder = 1
        else:
            SQLDateOrder = MdlADOFunctions.fGetRstValLong(res)
        res = QueryValue('emerald', 'UserTimeOut')
        if res == '':
            res = MdlConnection.SetUserTimeOut()
        UserTimeOut = int(res)
        # strMetaCn[0] = GetRegValuesToMetaConnection(ArrParams, FromWeb)
        # strCn[0] = GetRegValuesToConnection(strMetaCn, ArrParams, FromWeb)

        strMetaCn[0] = "Driver={SQL Server};Server=.;Database=LiadMeta;Trusted_Connection=yes"
        strCn[0] = "Driver={SQL Server};Server=.;Database=LiadData;Trusted_Connection=yes"

        strSChCon[0] = GetRegValuesToSchConnection(strMetaCn)
        strERPCon[0] = GetRegValuesToERPConnection(strMetaCn)
        returnVal = True
