...
from LiadPCUnite.BusinessLogic.Modules import MdlCommon as mdlCom
...
class Server:
    ...
    mSystemVariables = SystemVariables()
    mStartTime = datetime.date()
    ...
    def StartServer(self, Arr):
        try:
            strSQL = ''
            ServerName = ''
            Rst = qe.QueryExecutor(self.sqlCntr.GetConnection())
            DepRst = qe.QueryExecutor(self.sqlCntr.GetConnection())
            ...
            if len(Arr) >= 0:
                if Arr(5).isnumeric():
                    if adoFunc.fGetRstValLong(Arr(5)) > 0:
                        self.SCID = adoFunc.fGetRstValLong(Arr(5))
                    else:
                        self.SCID = 0
                else:
                    self.SCID = 0
            ...
            self.mStatus = 0
            self.mConCount = 1
            if not mdlCom.GetConnectionStrings(self.mCNstr, self.mMetaCNstr, strSchCN, Arr()):
                raise(1)
            if self.SCID != 0:
                tShiftCalendar = self.ShiftCalendar()
                tShiftCalendar.Init(self.SCID)
                self.ShiftCalendar = tShiftCalendar
            tDuplicateRTFound = fCheckForDuplicateRealTimes(self)
            if tDuplicateRTFound == True:
                frmMain.CloseNext = True
                RecordError('StartServer', '9999', 'Duplicate Realtimes Found', '')
                MsgBox('Duplicated RealTimes Found for this ShiftCalendar!!!', VBGetMissingArgument(MsgBox, 1), 'FATAL ERROR!')
                AllowClose = True
                ...
                if lhwndProcess != 0:
                    ...
                    TerminateProcess(lhwndProcess, 1)
                    CloseHandle(lhwndProcess)
                fn_return_value = False
                return fn_return_value
                
            else:
                self.ShiftCalendar.WindowsProcessID = GetCurrentProcessId
                self.ShiftCalendar.UpdateWindowsProcessID
            
            self.CleanActivateJobWebParams
            
            self.mOPCServer = OPCServer()
            ServerName = adoFunc.GetSingleValue('OPCServer', 'STblSystemVariables', 'ID = 1', 'CN')
            self.mOPCServer.Connect(ServerName)
            self.mOPCServerGroups = self.mOPCServer.OPCGroups
            GeneralGroupRefreshRate = 2000
            
            if self.mSCID > 0:
                CurShift = adoFunc.fGetRstValLong(adoFunc.GetSingleValue('CurrentShiftID', 'STblShiftCalendar', 'ID = ' + self.SCID, 'CN'))
            else:
                CurShift = adoFunc.fGetRstValLong(adoFunc.GetSingleValue('CurrentShiftID', 'STblSystemVariables', 'ID > 0', 'CN'))
            self.CurrentShiftID = CurShift
            strEncoding = adoFunc.GetSingleValue('Encoding', 'STblSystemVariables', 'ID > 0', 'CN')
            self.mWriteInterval = adoFunc.fGetRstValLong('' + adoFunc.GetSingleValue('WriteInterval', 'STblSystemVariables', 'ID > 0', 'CN'))
            strXMLHeader = '<?xml version=\'1.0\' encoding=\'' + strEncoding + '\' ?>' + '\r\n'
            AlarmOnProgressInterval = adoFunc.fGetRstValLong('' + adoFunc.GetSingleValue('SetupProgressInterval', 'STblSystemVariables', 'ID > 0', 'CN'))
            
            MultiTimeZoneLeaderServer = adoFunc.fGetRstValBool(adoFunc.GetSingleValue('MultiTimeZoneLeaderServer', 'STblSystemVariables', 'ID > 0', 'CN'), False)
            TargetDaylightSavingOn = adoFunc.fGetRstValBool(adoFunc.GetSingleValue('ThisDBDaylightSavingOn', 'STblSystemVariables', 'ID > 0', 'CN'), False)
            ThisDBGMT = adoFunc.fGetRstValDouble(adoFunc.GetSingleValue('ThisDBGMT', 'STblSystemVariables', 'ID > 0', 'CN'))
            if MultiTimeZoneLeaderServer and ThisDBGMT > - 720.1 and ThisDBGMT < 720.1:
                if TargetDaylightSavingOn:
                    GMTAdd = ThisDBGMT + 60
                else:
                    GMTAdd = ThisDBGMT
                GMTAdd = GMTAdd -  ( mdlCom.GetTimeDifference() / 60 )
            else:
                GMTAdd = 0
            
            if GMTAdd != 0:
                self.StartTime = mdlCom.DateAdd('n', ( GMTAdd ), self.StartTime)
            ...
            if self.mSCID > 0:
                ...
                DepRstData = DepRst.SelectAllData(strSQL)
                while not DepRstData.EOF:
                    
                    self.ShiftCalendar.AddDepartment(GetOrCreateDepartment(self, adoFunc.fGetRstValLong(DepRstData["ID"])))
                    strSQL = 'Select ID, OPCServerName, OPCServerIP From TblMachines Where IsActive <> 0 AND Department = ' + DepRstData["ID"] + ' ORDER BY ID'
                    ...
                    while not RstData.EOF:
                        ...
                        GetOrCreateDepartment(self, adoFunc.fGetRstValLong(DepRstData["ID"])).AddMachine(tMachine)
                        RstData.MoveNext()
                    RstData.Close()
                    DepRstData.MoveNext()
                DepRstData.Close()
                ...
                while not RstData.EOF:
                    ...
                    tMachine.Server = self
                    if tMachine.INITMachine(RstData["ID"], self.mOPCServer) == False:
    ...
    def CloseEvents(self, pCurrentShiftID):
        try:
            ...
            tPConfigPC = 0
            Rst = qe.QueryExecutor(self.sqlCntr.GetConnection())
            eRst = qe.QueryExecutor(self.sqlCntr.GetConnection())
            self.sqlCntr.OpenConnection()
            tMachine = Machine()
            ...
            while not RstData.EOF:    
                tEventID = 0
                tEventGroupID = 0
                tEndTime = mdlCom.NowGMT()
                tDuration = adoFunc.fGetRstValDouble(DateDiff('n', RstData["EventTime"], mdlCom.NowGMT()))
                RstData["Status"] = 3
                
                if fCheckCalendarForCloseEventsOnStart(adoFunc.fGetRstValLong(RstData["MachineID"]), CDate(RstData["EventTime"]), tDuration, tEventID, tEventGroupID) == True:
                    RstData["IsCalendarEvent"] = True
                    RstData["RootEventID"] = 0
                else:
                    tEventID = adoFunc.fGetRstValLong(RstData["event"])
                    tEventGroupID = adoFunc.fGetRstValLong(RstData["EventGroup"])
                tPConfigPC = adoFunc.fGetRstValDouble(RstData["PConfigPC"])
                if tPConfigPC == 0:
                    tPConfigPC = 100
                ...
                if tEventID == 0:
                    ...
                    if eRstData.RecordCount == 1:
                        tIsInActiveTime = adoFunc.fGetRstValBool(eRstData["IsInactiveTime"], False)
                        tIsDownTime = adoFunc.fGetRstValBool(eRstData["IsDownTime"], False)
                    eRstData.Close()
                    eRstData = None
                ...
                if adoFunc.fGetRstValLong(RstData["PConfigParentID"]) != 0:
                    tDuration = 0
    ...
    def fShiftTimer_Tick(self):
        ...
        fn_return_value = 'Shift ' + mdlCom.NowGMT + '; Error: ' + Err.Number + ':' + Err.Description
        return fn_return_value
    ...
    def XMLMain(self, McID):
        ...
        IsOnline = Date()
        tMachine = Machine()
        IsOnline = ShortDate(mdlCom.NowGMT(), False, True)
    ...
    def fReadTimer_Tick(self):
        ...
        tMachineName = ''
        strSQL = ''
        if DateDiff('n', self.ShiftCalendar.LastSQLReconnect, mdlCom.NowGMT) >= self.ShiftCalendar.SQLReconnectInterval:
            self.ShiftCalendar.LastSQLReconnect = mdlCom.NowGMT
            if CN.State == 1:
                CN.Close()
            if MetaCn.State == 1:
                MetaCn.Close()
        ...
        if Err.Number != 0:
            ...
            fn_return_value = 'Read: ' + mdlCom.NowGMT + '; Machine:' + self.ReadMachine + ' (' + tMachineName + '); Wait: ' + self.mReadWaitCount + '; Error: ' + Err.Number + ':' + Err.Description
            Err.Clear()
            
        fn_return_value = 'Read: ' + mdlCom.NowGMT + '; Machine:' + self.ReadMachine + ' (' + tMachineName + '); Wait: ' + self.mReadWaitCount + '; Error: ' + Err.Number + ':' + Err.Description
        strSQL = 'UPDATE STblShiftCalendar SET Descr = \'' + strFixBadChars(self.fReadTimer_Tick()) + '\', RTLastUpdate = \'' + ShortDate(mdlCom.NowGMT, True, True, True) + '\' Where ID = ' + self.SCID
        CN.Execute(( strSQL ))
        return fn_return_value

    def fWriteTimer_Tick(self):
        return 'Write ' + mdlCom.NowGMT + '; Error: ' + Err.Number + ':' + Err.Description
    ...
    def RunWebActions(self):
        ...
        while not Rst.EOF:
            ...
            if (select_variable_2 == 'ShiftChange'):
                ...
                Arg1 = CDate('' + Rst["Arg1"])
                Arg2 = adoFunc.fGetRstValLong(Rst["Arg2"])
                Arg3 = adoFunc.fGetRstValLong(Rst["Arg3"])
                Arg4 = adoFunc.fGetRstValLong(Rst["Arg4"])
                ActionOK = self.ShiftChange(Arg1, CLng(Arg2), CLng(Arg3), CLng(Arg4))
            elif (select_variable_2 == 'ChangeMoldCavities'):
                Arg1 = adoFunc.fGetRstValLong(Rst["Arg1"])
                Arg2 = '' + Rst["Arg2"]
                ...
                Arg2 = adoFunc.fGetRstValLong(Rst["Arg2"])
                Arg3 = adoFunc.fGetRstValBool(Rst["Arg3"], False)
                Arg4 = adoFunc.fGetRstValBool(Rst["Arg4"], False)
                Debug.Print('WEB: MachineLoadJob: MachineID=' + Arg1 + ' JobID=' + Arg2 + ' ResetTotals=' + Arg3 + ' FromActivateJob=' + Arg4 + ' | ' + mdlCom.NowGMT)
                ...
                Arg2 = adoFunc.fGetRstValBool(Rst["Arg1"], True)
                Arg1 = adoFunc.fGetRstValLong(Rst["Arg2"])
                ...
                if not ( self.SCID == fGetShiftCalendarIDByMachine(CLng(Arg1)) and self.SCID != 0 ) :
                    ...
                    GoTo(line1)
                Arg2 = adoFunc.fGetRstValBool(Rst["Arg2"], False)
                ...
                Arg4 = adoFunc.fGetRstValLong(Abs(adoFunc.fGetRstValBool(Rst["Arg4"], False)))
                Arg5 = adoFunc.fGetRstValLong(Rst["Arg5"])
                ...
                Arg2 = adoFunc.fGetRstValLong(Rst["Arg1"])
                Arg1 = adoFunc.fGetRstValLong(Rst["Arg2"])
                ...
                if not ( self.SCID == fGetShiftCalendarIDByMachine(CLng(Arg1)) and self.SCID != 0 ) :
                    GoTo(line1)
                Arg2 = adoFunc.fGetRstValLong(Rst["Arg2"])
                Arg3 = adoFunc.fGetRstValDouble(Rst["Arg3"])
                ...
                Arg2 = adoFunc.fGetRstValLong(Rst["Arg2"])
                Arg3 = adoFunc.fGetRstValLong(Rst["Arg3"])
                Arg4 = adoFunc.fGetRstValDouble(Rst["Arg4"])
                ...
                Arg3 = adoFunc.fGetRstValLong(Rst["Arg3"])
                Arg4 = adoFunc.fGetRstValLong(Rst["Arg4"])
                self.ActivateLocationOnChannelSplit(adoFunc.fGetRstValLong(Arg1), adoFunc.fGetRstValLong(Arg2), adoFunc.fGetRstValLong(Arg3), adoFunc.fGetRstValLong(Arg4))
                ...
                Arg3 = adoFunc.fGetRstValLong(Rst["Arg3"])
                Arg4 = adoFunc.fGetRstValLong(Rst["Arg4"])
                self.LinkLocationToChannelSplit(adoFunc.fGetRstValLong(Arg1), adoFunc.fGetRstValLong(Arg2), adoFunc.fGetRstValLong(Arg3), adoFunc.fGetRstValLong(Arg4))
                ...
                ActionOK = self.SetMachineFieldLValue(CLng(Arg1), str(Arg2), adoFunc.fGetRstValDouble(Arg3))
                RecalcMachine = False
                MachineID = CLng(Arg1)
                ...
                ActionOK = self.SetMachineFieldHValue(CLng(Arg1), str(Arg2), adoFunc.fGetRstValDouble(Arg3))
                RecalcMachine = False
                MachineID = CLng(Arg1)
    ...
    def MachineJoshDetailsCalc(self, pMachineID, pJoshID, pCalcDuration=0, pCalcMat=0, pLiveCalc=0):
        ...
        fn_return_value = False
        tMachine = self.mMachines.Item(str(pMachineID))
        tShiftID = adoFunc.fGetRstValLong(adoFunc.GetSingleValue('ShiftID', 'TblJosh', 'ID = ' + pJoshID, 'CN'))
        if not tMachine.ActiveJob is None:
            if not tMachine.ActiveJob.ActiveJosh is None:
            ...
            tJobID = adoFunc.fGetRstValLong(adoFunc.GetSingleValue('JobID', 'TblJosh', 'ID = ' + pJoshID))
            tJob = Job()
    ...
    def CleanUnusedObjects(self):
        ...
        strSQL = 'DELETE STblUtilizationLogs WHERE (LogTime <= mdlCom.DateAdd(DD, -180, GETDATE()))'
        CN.Execute(strSQL)
        fn_return_value = True
        if Err.Number != 0:
            if InStr(Err.Description, 'nnection') > 0:
    ...
    def fClearAlarmsOnServerINIT(self, pShiftCalendarID):
        ...
        strSQL = strSQL + '('
        strSQL = strSQL + 'SELECT ID FROM STblDepartments WHERE ShiftCalendarID = ' + pShiftCalendarID
        strSQL = strSQL + ')'
        strSQL = strSQL + ')'
        strSQL = strSQL + ')'
        CN.Execute(strSQL)
    ...
    def ClosePreviousShifts(self):
        ...
        if self.CurrentShiftID != 0:
            ...
            RstData = Rst.SelectAllData(strSQL)
            while not RstData.EOF:
                tStartTime = adoFunc.GetSingleValue('TOP 1 StartTime', 'TblShift', 'ShiftCalendarID = ' + self.SCID + ' AND ID > ' + RstData["ID"] + ' ORDER BY ID')
                if tStartTime != 0:
    ...
    def StartMachineMaterialFlow(self, pMachineID, pJobID):
        ...
        if not ( tMachine is None ) :
            if tMachine.ActiveJob.ID == pJobID:
                tMachine.ActiveJob.NextJobMaterialFlowStart = mdlCom.NowGMT
    ...
    def UpdateMachineProductionMode(self, pMachineID, pProductionModeID):
        ...
        PRst = qe.QueryExecutor(self.sqlCntr.GetConnection())
        ...
        if PRst.RecordCount == 1:
            tMachine.ProductionModeReasonID = adoFunc.fGetRstValLong(PRst["EventReasonID"])
            tMachine.ProductionModeGroupReasonID = adoFunc.fGetRstValLong(adoFunc.GetSingleValue('EventGroupID', 'STblEventDesr', 'ID = ' + tMachine.ProductionModeReasonID))
            tMachine.ProductionModeDisableProductionTime = adoFunc.fGetRstValBool(PRst["DisableProductionTime"], False)
            tMachine.ProductionModeCalcEfficiencies = adoFunc.fGetRstValBool(PRst["CalcEfficiencies"], False)
            tMachine.ProductionModeOverCalendarEvent = adoFunc.fGetRstValBool(PRst["OverCalendarEvent"], True)
        PRst.Close()
        fn_return_value = True
        if Err.Number != 0:
            if InStr(Err.Description, 'nnection') > 0:
    ...
    def CleanActivateJobWebParams(self):
        ...
        CN.Execute(strSQL)
        strSQL = 'DELETE FROM STblWebParams WHERE FuncName = \'CloseRealTime\' AND Arg1 = \'' + self.SCID + '\''
        CN.Execute(strSQL)
        if Err.Number != 0:
            if InStr(Err.Description, 'nnection') > 0:
                if CN.State == 1:
        ...
        for tVariant in self.Machines:
            ...
            if not tMachine.ActiveJob is None:
                ...
                if tMachine.ActiveJob.ActiveJosh.ID == 0:
                    tMachine.ActiveJoshID = adoFunc.fGetRstValLong(adoFunc.GetSingleValue('ID', 'TblJoshCurrent', 'JobID = ' + tMachine.ActiveJob.ID + ' AND ShiftID <> ' + self.CurrentShiftID + ' ORDER BY StartTime DESC', 'CN'))
                    tJosh = Josh()
                ...
                if tMachine.ActiveJob.PConfigID != 0 and tMachine.ActiveJob.IsPConfigMain == True:
                    for tVariant2 in tMachine.ActiveJob.PConfigJobs:
                        ...
                        if tChildJob.ActiveJosh is None:
                            JoshID = adoFunc.fGetRstValLong(adoFunc.GetSingleValue('ID', 'TblJoshCurrent', 'JobID = ' + tChildJob.ID + ' AND ShiftID <> ' + self.CurrentShiftID + ' ORDER BY StartTime DESC', 'CN'))
                            if JoshID != 0:
                                tJosh = Josh()
